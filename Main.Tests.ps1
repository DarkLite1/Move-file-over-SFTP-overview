...
Describe 'when the input file is valid' {
    ...
    Context 'with a single task and single path' {
        ...
        AfterAll {
            Get-ChildItem -Path $testInputFile.Settings.SaveLogFiles.Where.Folder -File |
            Remove-Item -Force
            Get-ChildItem -Path $testJsonFolder -File | Remove-Item -Force
        }
    }
...
Describe 'SendMail.When' {
    ...
    AfterAll {
        ...
        Remove-Item -Force
        Get-ChildItem -Path $testJsonFolder -File -EA Ignore | Remove-Item -Force
    }
}
